import { Typography, Space, Tag, Card, Button, Divider, Row, Col } from 'antd';
import {
  GithubOutlined, ArrowRightOutlined,
  DesktopOutlined, AppleOutlined, WindowsOutlined, LinuxOutlined,
...
export default function About() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 16px' }}>
      ...
      <Card
        style={{
          borderRadius: 16, textAlign: 'center', padding: '8px 0',
      ...
      {/* Description */}
      <Card
        style={{ borderRadius: 12, marginTop: 8, border: `1px solid var(--border-subtle)` }}
      ...
      <Row gutter={[12, 12]} style={{ marginTop: 12 }}>
        {[
          ...
          <Col span={8} key={i}>
            <Card
              hoverable
              style={{ borderRadius: 12, height: '100%', border: `1px solid var(--border-subtle)` }}
              styles={{ body: { padding: '16px 20px' } }}
            >
              <Text strong style={{ fontSize: 13 }}>{f.title}</Text>
              <div style={{ marginTop: 6 }}>
                <Text style={{ fontSize: 12, color: 'var(--text-tertiary)', lineHeight: 1.6 }}>
                  {f.desc}
                </Text>
              </div>
            </Card>
      ...
      {/* Platform support */}
      <Card
        title={<Text strong style={{ fontSize: 14 }}>{t('about_platforms')}</Text>}
        ...
        <Space size={16} wrap>
          {platforms.map(p => (
            <Space key={p.label} size={8}>
              ...
              <Text style={{ fontSize: 13, opacity: p.status === 'ready' ? 1 : 0.45 }}>
                {p.label}
              </Text>
              {p.status === 'ready' && (
      ...
      >
        <Space style={{ width: '100%', justifyContent: 'space-between' }} align="center">
          ...
          <Space>
            <Button disabled icon={<DesktopOutlined />} style={{ borderRadius: 8 }}>
              {t('about_check_update')}
            </Button>
            <Button type="primary" disabled icon={<ArrowRightOutlined />} style={{ borderRadius: 8 }}>
              {t('about_download')}
            </Button>
          </Space>
        </Space>
