...
impl OutputProtocol for ChatOutput {
    ...
    fn build_request(&self, req: &ProtocolRequest) -> anyhow::Result<Value> {
        ...
        if !system_parts.is_empty() {
            messages
                .push(serde_json::json!({"role": "system", "content": system_parts.join("\n\n")}));
        }
        ...
        if let Some(maxt) = req.max_tokens {
            result["max_tokens"] = Value::Number(serde_json::Number::from(maxt));
        }
        if let Some(temp) = req.temperature {
            result["temperature"] =
                Value::Number(serde_json::Number::from_f64(temp as f64).unwrap());
        }
        if let Some(top) = req.top_p {
            result["top_p"] = Value::Number(serde_json::Number::from_f64(top as f64).unwrap());
        }
        ...
        if req
            .tools
            .iter()
            .any(|t| matches!(t.tool_type, ToolType::Function))
        {
            if let Some(ref tc) = req.tool_choice {
                result["tool_choice"] = tc.clone();
            }
        ...
        if let Some(fp) = req.passthrough.frequency_penalty {
            result["frequency_penalty"] =
                Value::Number(serde_json::Number::from_f64(fp as f64).unwrap());
        }
        if let Some(pp) = req.passthrough.presence_penalty {
            result["presence_penalty"] =
                Value::Number(serde_json::Number::from_f64(pp as f64).unwrap());
        }
    ...
    fn build_response(&self, resp: &ProtocolResponse) -> anyhow::Result<Value> {
        ...
        let content = if output_items.is_empty() {
            ""
        } else {
            &output_items.join("\n")
        };
        ...
        .map(|p| match p {
            ...
            ContentPart::Image { url, detail } => {
                ...
                if let Some(d) = detail {
                    img["image_url"]["detail"] = Value::String(d.clone());
                }
                img
            }
...
/// 统一修复工具parameters，确保type="object"，兼容null参数、type:null非法结构
fn fix_tool_parameters(raw_params: &Value) -> Value {
    let mut params = raw_params.clone();
    // 如果是对象，强制修正type字段
...
fn build_chat_tool(tool: &ToolDefinition) -> Value {
    // Ensure tool name is never empty — DeepSeek/generic Chat APIs reject empty names
    let name = if tool.name.is_empty() {
        match &tool.tool_type {
            ToolType::WebSearch => "web_search",
            ToolType::LocalShell => "local_shell",
            ToolType::ComputerUse => "computer_use",
            ToolType::Custom(_) => "custom_tool",
            _ => "function_tool",
        }.to_string()
    } else {
        tool.name.clone()
    ...
    match &tool.tool_type {
        ToolType::Function => {
            let params = fix_tool_parameters(&tool.parameters);
        ...
        | ToolType::ComputerUse => {
            let desc = tool.description.clone().unwrap_or_else(|| {
                format!(
